...
async function extractInvoiceData(
  ...
  const preferredModel = process.env.OPENAI_MODEL ?? "gpt-4";
  ...
  try {
    return await callModel(preferredModel);
  } catch (err: any) {
    // If model not found / access error, try fallback
    const code = err?.code ?? err?.status ?? null;
    if (code === "model_not_found" || err?.status === 404) {
      console.warn(
        `Model ${preferredModel} not available, retrying with ${fallbackModel}`,
      );
